...
const getAllitem = async () => {
  const data = await fetch("http://localhost:5000/EnDeploma", { cache: "no-cache" });
  return data.json();
}

const EnDeploma = async () => {

  const EnDeploma = await getAllitem();

  const handleDelete = async (id) => {
    await axios.delete(`http://localhost:5000/EnDeploma/${id}`)
