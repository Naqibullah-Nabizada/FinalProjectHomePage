...
const Update = () => {
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/TwelveSection/guestHouse/update/${id}`, {
  ...
  const getSingle = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/TwelveSection/guestHouse/${id}`);
      setName(data.name);
    ...
    <>
      <header>
        <h3 className="my-4 text-center text-xl">
          فورم ویرایش کرایه مهمانخانه آمریت خدمات
        </h3>
      </header>
      <hr />
      <main>
