...
const Update = () => {
  ...
  const [type, setType] = useState("animalClinic");
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/TwelveSection/animal-clinic/update/${id}`, {
  ...
  const getSingle = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/TwelveSection/animal-clinic/${id}`);
      setName(data.name);
