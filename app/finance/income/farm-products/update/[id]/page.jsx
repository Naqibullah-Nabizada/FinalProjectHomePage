...
const Update = () => {
  ...
  const [type, setType] = useState("farmaticProducts");
  ...
  const submitForm = async (e) => {
    e.preventDefault();
    try {
      router.push("/finance/income/farm-products");
      await axios.put(`http://localhost:5000/TwelveSection/farmaticProducts/update/${id}`, {
  ...
  const getSingle = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/TwelveSection/farmaticProducts/${id}`);
      setName(data.name);
    ...
    <>
      <header>
        <h3 className="my-4 text-center text-xl">
          فورم ویرایش محصولات تجزیه فارمسوتیکی
        </h3>
      </header>
      <hr />
      <main>
        ...
        <section className="w-[95%] flex justify-between flex-wrap mx-auto my-3">
          ...
          <div className="flex">
            ...
            <Link href="./finance/income/farm-products" className="btn btn-outline-secondary flex">
