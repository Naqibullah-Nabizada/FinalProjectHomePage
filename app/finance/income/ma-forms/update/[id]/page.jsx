...
const Update = () => {
  ...
  const [type, setType] = useState("MAForms");
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/TwelveSection/MAForms/update/${id}`, {
  ...
  const getSingle = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/TwelveSection/MAForms/${id}`);
      setName(data.name);
    ...
    <>
      <header>
        <h3 className="my-4 text-center text-xl">
          فورم ویرایش فیس محصلین برنامه های ماستری
        </h3>
      </header>
      <hr />
      <main>
