...
const Update = () => {
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/TwelveSection/hostelBread/update/${id}`, {
  ...
  const getSingle = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/TwelveSection/hostelBread/${id}`);
      setName(data.name);
    ...
    <>
      <header>
        <h3 className="my-4 text-center text-xl">
          فورم ویرایش فروش نان قاق لیلیه
        </h3>
      </header>
      <hr />
      <main>
