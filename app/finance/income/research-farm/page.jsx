  ...
  return (
    <>
      <header className="flex" id="header">
        <Header hrefAddBtn={admin == 0 ? ("/finance/income/research-farm/add") : ""} hrefBackBtn="/finance/income" section={"income"} pageName="research-farm" />
      </header>
      <hr />
      <main className="w-[99%] mx-auto" id="main">
        <table className="table table-bordered table-sm table-striped">
          <caption className="caption-top text-center text-dark">عواید فارم تحقیقاتی پوهنځی زراعت پوهنتون کابل</caption>
          ...
          <tbody>
            {
              researchFarm.map((item) => (
                <tr key={item.id} style={item.pendant_num == null || item.pendant_num == '' ? { background: "#F2BBA7" } : null}>
                  ...
                  <td>{item.remark}</td>
                  {
                    admin == 0 ? (<td className="flex justify-around" id="edit_btn">
