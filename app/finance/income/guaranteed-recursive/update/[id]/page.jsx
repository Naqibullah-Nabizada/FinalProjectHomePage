...
const Update = () => {
  ...
  const [type, setType] = useState("guaranteedRecursive");
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/TwelveSection/guaranteedRecursive/update/${id}`, {
  ...
  const getSingle = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/TwelveSection/guaranteedRecursive/${id}`);
      setName(data.name);
