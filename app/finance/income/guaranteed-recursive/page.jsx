...
const GuaranteedRecursive = () => {
  ...
  const { totalAmount } = guaranrRecursive.reduce((accumulator, item) => {
    return {
  ...
  return (
    <>
      <header className="flex" id="header">
        <Header hrefAddBtn={admin == 0 ? ("/finance/income/guaranteed-recursive/add") : ""} hrefBackBtn="/finance/income" section={"income"} pageName="guaranteed-recursive" />
      </header>
      <hr />
      <main className="w-[99%] mx-auto" id="main">
        <table className="table table-bordered table-sm table-striped">
          <caption className="caption-top text-center text-dark">عواید تضمین و بازگشتی پوهنتون کابل</caption>
