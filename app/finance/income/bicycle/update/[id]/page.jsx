...
const Update = () => {
  ...
  const [type, setType] = useState("bicycle");
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/TwelveSection/bicycle/update/${id}`, {
  ...
  const getSingle = async () => {
    try {
      const { data } = await axios.get(`http://localhost:5000/TwelveSection/bic/${id}`);
      setName(data.name);
