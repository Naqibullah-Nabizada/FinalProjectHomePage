...
const MAFees = () => {
  ...
  const { totalAmount } = MAFees.reduce((accumulator, item) => {
    return {
  ...
  return (
    <>
      <header className="flex" id="header">
        <Header hrefAddBtn={admin == 0 ? ("/finance/income/ma-fees/add") : ""} hrefBackBtn="/finance/income" section={"income"} pageName="ma-fees" />
      </header>
      <hr />
      <main className="w-[99%] mx-auto" id="main">
        <table className="table table-bordered table-sm table-striped">
          <caption className="caption-top text-center text-dark">عواید فیس محصلین برنامه های ماستری پوهنتون کابل</caption>
